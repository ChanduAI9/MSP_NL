...
class UserRegistrationView(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        user = authenticate(self.request, username=instance.username, password=self.request.data.get('password'))
        if user:
...
class FinancialDetailsView(TemplateView, generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if FinancialDetails.objects.filter(user=request.user).exists():
            return redirect('calculate-metrics')
            ...
            serialized_results = [
                {
                    ...
                    'CMP Rs': entry.cmp_rs,
                    'P/E': entry.pe,
                    'Mar Cap Rs Cr': entry.mar_cap_rs,
                    'No of Shares (in Cr)': entry.num_of_shares,
                    'NP Qtr Rs Cr': entry.np_qtr_rs,
                    'Sales Qtr Rs Cr': entry.sales_qtr_rs,
                    'ROCE %': entry.roce_percentage,
                    'Sales Rs Cr': entry.sales_rs,
                    'Sales per share': entry.sales_per_share,
                    'CMP/Sales': entry.cmp_over_sales,
                    'CMP / BV': entry.cmp_over_bv,
                    'BV in Cr': entry.bv_in_cr,
                    'Ind PE': entry.ind_pe,
                    'Profit growth %': entry.profit_growth_percentage,
                    'EV Rs Cr': entry.ev_rs,
                    'PAT 12M Rs Cr': entry.pat_12m_rs,
                    'Debt / Eq': entry.debt_eq,
                    'CMP / OCF': entry.cmp_over_ocf,
                    'EV / EBITDA': entry.ev_over_ebitda,
